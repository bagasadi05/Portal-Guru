...
export const CardHeader: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({ className, ...props }) => (
  <div
    className={['p-4 md:p-6 border-b border-gray-200 dark:border-white/10 relative', className].filter(Boolean).join(' ')}
    {...props}
  />
);
...
export const CardFooter: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({ className, ...props }) => (
  <div
    className={['p-4 md:p-6 border-t border-gray-200 dark:border-white/10', className].filter(Boolean).join(' ')}
    {...props}
  />
);