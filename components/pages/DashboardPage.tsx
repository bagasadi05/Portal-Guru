...
type CardColor = 'blue' | 'yellow' | 'purple';
...
const fetchDashboardData = async (userId: string): Promise<DashboardQueryData> => {
    ...
    const dailySummaryData = (dailySummaryRes.data as any)?.[0];
    ...
    return {
        ...
        weeklyAttendance: (weeklyAttendanceRes.data as any) || [],
...
const colorConfig: Record<CardColor, { color: string; shadow: string; borderColor: string; }> = {
    blue: {
...
const InteractiveStatCard: React.FC<{
    ...
    icon: React.ElementType;
    color: CardColor;
}> = ({ link, label, value, details, icon: Icon, color }) => {
    const { color: colorClass, shadow, borderColor } = colorConfig[color];
    return (
        <Link to={link} className="block group h-full relative">
            <Card className={`text-white overflow-hidden relative transition-all duration-300 group-hover:-translate-y-2 shadow-lg ${shadow} h-full flex flex-col justify-center ${borderColor}`}>
                <div className={`absolute inset-0 bg-gradient-to-br ${colorClass} opacity-40 group-hover:opacity-60 transition-opacity duration-300`}></div>
...
const DashboardPage: React.FC = () => {
    ...
    const stats: Stat[] = [
        { label: 'Total Siswa', value: students.length, icon: UsersIcon, color: 'blue', link: '/siswa', details: <p className="text-sm font-medium">{students.filter(s => s.gender === 'Laki-laki').length} Laki-laki<br/>{students.filter(s => s.gender === 'Perempuan').length} Perempuan</p> },
        { label: 'Laporan Baru', value: newReportsCount, icon: BookOpenIcon, color: 'yellow', link: '/siswa', details: <p className="text-sm font-medium leading-tight">Laporan baru dalam 7 hari terakhir</p> },
        { label: 'Kelas Diajar', value: classCount, icon: CalendarIcon, color: 'purple', link: '/jadwal', details: <p className="text-sm font-medium leading-tight">Total kelas yang Anda ajar</p> },
    ];
    
    return (
        <div className="w-full min-h-full p-4 sm:p-6 md:p-8 flex flex-col space-y-8">
            {showGreeting && user && <GreetingRobot userName={user.name} onAnimationEnd={() => setShowGreeting(false)} />}

            <div className="animate-fade-in-up">
                <h1 className="text-3xl md:text-4xl font-bold text-white text-shadow-md">Selamat Datang, {user?.name}!</h1>
                <p className="mt-2 text-indigo-200 max-w-2xl">Dasbor ringkasan untuk aktivitas mengajar Anda hari ini.</p>
            </div>
            ...
            <div className="animate-fade-in-up" style={{ animationDelay: '400ms' }}>
                 <Card className="overflow-hidden">
                    ...
                    <div className="relative z-10">
                        <AiDashboardInsight students={students} attendance={dailyAttendanceSummary} academicRecords={academicRecords} violations={violations} />
                    </div>
                </Card>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-8">
                    <Card className="animate-fade-in-up" style={{ animationDelay: '500ms' }}><CardHeader><CardTitle>Ringkasan Kehadiran Mingguan</CardTitle><CardDescription>Grafik kehadiran siswa selama seminggu terakhir.</CardDescription></CardHeader><CardContent className="pb-10"><WeeklyAttendanceChart attendanceData={weeklyAttendance} /></CardContent></Card>
                ...
                <div className="lg:col-span-1">
                    <Card className="flex flex-col animate-fade-in-up h-full" style={{ animationDelay: '700ms' }}><CardHeader><CardTitle>Jadwal Hari Ini</CardTitle></CardHeader>
