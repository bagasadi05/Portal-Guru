...
const MassInputPage: React.FC = () => {
    ...
    useEffect(() => {
        if (location.state?.prefill) {
            const { mode: preMode, classId, subject, assessment_name } = location.state.prefill;
            if (preMode) {
                setMode(preMode);
                setStep(2);
            }
    ...
    const { data: uniqueSubjects } = useQuery({
        ...
        queryFn: async (): Promise<string[]> => {
            ...
            // FIX: Explicitly cast the Supabase response data to ensure correct typing and resolve the 'unknown[]' assignment error.
    ...
    useEffect(() => {
        if (mode === 'subject_grade' && existingGrades) {
            // FIX: Explicitly typed the `record` parameter to fix type inference issues where `record` could be `unknown`, causing an error when accessing `record.student_id`.
    ...
    // FIX: Added explicit return type for useMemo to prevent students being inferred as 'unknown[]' when studentsData is undefined.
    ...
    const handlePrintGrades = async () => {
        ...
        // Get unique assessment names from all records for the subject
        const allAssessments = [...new Set(academicRecords?.map(r => r.assessment_name || 'Lainnya'))].sort();
        ...
        <div className="w-full min-h-screen p-4 sm:p-6 md:p-8 pb-24 flex flex-col cosmic-bg text-white overflow-y-auto">
            {step === 1 ? <Step1_ModeSelection handleModeSelect={handleModeSelect} /> : (
                <div className="w-full max-w-7xl mx-auto flex flex-col flex-grow">
                    ...
                    </div></div>{mode === 'subject_grade' && isOnline && (<div className="bg-white/5 backdrop-blur-lg rounded-2xl border border-white/10 p-6"><h3 className="font-bold text-lg mb-4 border-b border-white/10 pb-3 flex items-center gap-2"><SparklesIcon className="w-5 h-5 text-purple-400"/>Tempel Data Nilai</h3><textarea value={pasteData} onChange={e => setPasteData(e.target.value)} placeholder="Contoh:&#10;Budi Santoso   95&#10;Ani Wijaya      88" rows={4} className="w-full p-2 border rounded-md bg-white/10 border-white/20 text-white placeholder:text-gray-400"></textarea><Button onClick={handleAiParse} disabled={isParsing} className="mt-2"><ClipboardPasteIcon className="w-4 h-4 mr-2"/>{isParsing ? 'Memproses...' : 'Proses dengan AI'}</Button></div>)}</div><div className="lg:col-span-2 bg-white/5 backdrop-blur-lg rounded-2xl border border-white/10 flex flex-col"><div className="p-4 sm:p-6 border-b border-white/10 flex-shrink-0 flex flex-col sm:flex-row items-start sm:items-center gap-4 justify-between"><div className="relative flex-grow w-full sm:w-auto"><SearchIcon className="w-5 h-5 text-gray-400 absolute top-1/2 left-3 -translate-y-1/2"/><Input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Cari nama siswa..." className="pl-10 w-full"/></div><FilterPills options={filterOptions} currentValue={studentFilter} onFilterChange={setStudentFilter} /></div><div className="flex-grow overflow-y-auto p-4">{isLoadingStudents ? <p className="p-6 text-center">Memuat siswa...</p> : students && students.length > 0 ? (<><div className="hidden md:block overflow-x-auto"><table className="w-full text-sm striped-table sticky-header"><thead><tr><th className="p-4 text-left w-10">{mode !== 'subject_grade' && <Checkbox checked={isAllSelected} onChange={e => handleSelectAllStudents(e.target.checked)}/>}</th><th className="p-4 text-left font-semibold">Nama Siswa</th><th className="p-4 text-left font-semibold">{mode === 'subject_grade' ? 'Input Nilai' : (mode === 'academic_print' || mode === 'delete_subject_grade') ? 'Nilai Saat Ini' : 'Status'}</th></tr></thead><tbody>{students.map((s: StudentRow) => { const isSelected = selectedStudentIds.has(s.id); const gradeRecord = (mode === 'delete_subject_grade' || mode === 'academic_print') ? (existingGrades || []).find(g => g.student_id === s.id) : null; const hasGrade = !!gradeRecord; return(<tr key={s.id} onClick={mode !== 'subject_grade' ? () => handleStudentSelect(s.id) : undefined} className={`border-b border-white/10 transition-colors ${(isSelected || (mode === 'subject_grade' && scores[s.id]?.trim())) ? 'bg-purple-500/10' : 'hover:bg-white/5'} ${mode !== 'subject_grade' ? 'cursor-pointer' : ''}`}><td className="p-4">{mode !== 'subject_grade' && <Checkbox checked={isSelected} onChange={() => handleStudentSelect(s.id)} disabled={mode === 'delete_subject_grade' && !hasGrade}/>}</td><td className="p-4 flex items-center gap-3"><img src={s.avatar_url} alt={s.name} className="w-10 h-10 rounded-full object-cover ring-2 ring-white/10"/><span className="font-medium">{s.name}</span></td><td className="p-4">{mode === 'subject_grade' ? <Input type="number" min="0" max="100" value={scores[s.id] || ''} onChange={e => handleScoreChange(s.id, e.target.value)} placeholder="0-100" className="w-28"/> : (mode === 'academic_print' || mode === 'delete_subject_grade') ? <span className={`font-bold px-3 py-1.5 rounded-lg ${hasGrade ? 'bg-purple-500/20 text-purple-200' : 'bg-white/10 text-gray-500'}`}>{hasGrade ? gradeRecord.score : 'N/A'}</span> : isSelected ? <span className="text-green-400 font-semibold">Terpilih</span> : <span className="text-gray-500">Belum dipilih</span>}</td></tr>)})}</tbody></table></div><div className="md:hidden space-y-3">{students.map((s: StudentRow) => { const isSelected = selectedStudentIds.has(s.id); const gradeRecord = (mode === 'delete_subject_grade' || mode === 'academic_print') ? (existingGrades || []).find(g => g.student_id === s.id) : null; const hasGrade = !!gradeRecord; return(<div key={s.id} onClick={mode !== 'subject_grade' ? () => handleStudentSelect(s.id) : undefined} className={`bg-white/5 backdrop-blur-sm rounded-xl border transition-all duration-200 ${(isSelected || (mode === 'subject_grade' && scores[s.id]?.trim())) ? 'border-purple-500/50 bg-purple-500/10 shadow-lg shadow-purple-500/20' : 'border-white/10 hover:border-white/20'} ${mode !== 'subject_grade' ? 'cursor-pointer active:scale-98' : ''} p-4`}><div className="flex items-center gap-3 mb-3">{mode !== 'subject_grade' && <Checkbox checked={isSelected} onChange={() => handleStudentSelect(s.id)} disabled={mode === 'delete_subject_grade' && !hasGrade}/>}<img src={s.avatar_url} alt={s.name} className="w-12 h-12 rounded-full object-cover ring-2 ring-white/20 shadow-md"/><div className="flex-grow"><p className="font-semibold text-white text-base leading-tight">{s.name}</p><p className="text-xs text-gray-400 mt-0.5">No. {students.indexOf(s) + 1}</p></div></div>{mode === 'subject_grade' ? (<div className="flex items-center gap-3 mt-3 pt-3 border-t border-white/10"><label className="text-sm font-medium text-gray-300 min-w-[60px]">Nilai:</label><Input type="number" min="0" max="100" value={scores[s.id] || ''} onChange={e => handleScoreChange(s.id, e.target.value)} placeholder="0-100" className="flex-grow text-lg font-semibold text-center"/>{scores[s.id] && (<span className={`px-3 py-1.5 rounded-lg text-sm font-bold ${parseInt(scores[s.id]) >= 75 ? 'bg-green-500/20 text-green-300' : parseInt(scores[s.id]) >= 60 ? 'bg-yellow-500/20 text-yellow-300' : 'bg-red-500/20 text-red-300'}`}>{parseInt(scores[s.id]) >= 75 ? 'Baik' : parseInt(scores[s.id]) >= 60 ? 'Cukup' : 'Kurang'}</span>)}</div>) : (mode === 'academic_print' || mode === 'delete_subject_grade') ? (<div className="flex items-center justify-between mt-3 pt-3 border-t border-white/10"><span className="text-sm text-gray-400">Nilai Saat Ini:</span><span className={`font-bold px-3 py-1.5 rounded-lg text-lg ${hasGrade ? 'bg-purple-500/20 text-purple-200' : 'bg-white/10 text-gray-500'}`}>{hasGrade ? gradeRecord.score : 'N/A'}</span></div>) : (<div className="flex items-center justify-between mt-3 pt-3 border-t border-white/10"><span className="text-sm text-gray-400">Status:</span>{isSelected ? <span className="text-green-400 font-semibold flex items-center gap-1"><CheckSquareIcon className="w-4 h-4"/>Terpilih</span> : <span className="text-gray-500">Belum dipilih</span>}</div>)}</div>)})}</div></>) : <p className="p-6 text-center">Tidak ada siswa di kelas ini atau tidak ada hasil pencarian.</p>}</div></div></div>
